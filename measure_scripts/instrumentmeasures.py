...
def readNRP2(SAB, NRP2, misure_number, misure_delay, calibration_frequency, calibration_frequency_unit,
             SAB_switch_delay, make_zero=False):
    result_values = []
    ...
    for i in range(0, misure_number):
        ...
        if make_zero:
            ...
            command = "SENS1:FREQ:FIX " + str(calibration_frequency) + unit.return_unit_str(
                calibration_frequency_unit)  # Ex. FREQ 500kHz
            NRP2.write(command)
            ...
            SAB.write("SWT1 2")
            time.sleep(SAB_switch_delay)
        #time.sleep(misure_delay/1000.)
        #command = "SENS:AVER:COUN 1"
        #result = NRP2.write(command)
        #command = "TRAC:MEAS:POW:MAX?"
        command = "READ?"
...
def readFSV_marker(FSV,
    ...
    result += [str(x) for x in calibrate(tmp, frequency, spurius_IF_unit, calibration_IF, round_freq=True,
                                         calibration_function=calibration_IF_function,
                                         calibration_function_unit=calibration_IF_function_unit)]
    return result
...
def FSV_reset_setup(FSV, spectrum_analyzer_sweep_points,
    ...
    FSV.write("BAND " + str(spectrum_analyzer_resolution_bandwidth) + unit.return_unit_str(
        spectrum_analyzer_resolution_bandwidth_unit))  # set the resolution bandwidth
    FSV.write("BAND:VID " + str(spectrum_analyzer_video_bandwidth) + unit.return_unit_str(
        spectrum_analyzer_video_bandwidth_unit))  # set video bandwidth
    FSV.write("FREQ:SPAN " + str(spectrum_analyzer_frequency_span) + unit.return_unit_str(
        spectrum_analyzer_frequency_span_unit))  #
