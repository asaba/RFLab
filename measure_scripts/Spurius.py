...
def measure_LNA_spurius(SMB_LO, SMB_RF, FSV,
    ...
    for f_LO in frequency_LO_range: #frequency loop
        ...
        for l_LO in level_LO_range: #power level loop
            ...
            current_LO_level, calibration_LO_result =  calibrate(l_LO, f_LO,  unit.Hz, calibration_LO, calibration_function = calibration_function_LO, calibration_function_unit = calibration_function_LO_unit)
            ...
            for l_RF in level_RF_range: #power level loop
                ...
                for f_RF in frequency_RF_range: #frequency loop
                    ...
                    current_RF_level, calibration_RF_result =  calibrate(l_RF, f_RF, unit.Hz, calibration_RF, calibration_function = calibration_function_RF, calibration_function_unit = calibration_function_RF_unit)
                    ...
                    if not createprogressdialog is None:
                        import wx
                        wx.MicroSleep(500)
                        message = "LO {lo_freq} {lo_pow}dB - RF  {rf_freq} {rf_pow}dB".format(lo_freq = current_lo_frequency_human_readable, lo_pow = str(current_LO_level), rf_freq = current_rf_frequency_human_readable, rf_pow = str(current_RF_level))
                    ...
                    for s in spurius_markers:  
                        ...
                        values.append([f_LO_value, unit.return_unit_str(unit.Hz), str(l_LO), calibration_LO_result, f_RF_value, unit.return_unit_str(unit.Hz), str(l_RF), calibration_RF_result] + s)
