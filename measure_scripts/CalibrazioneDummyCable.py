...
def create_calibration_cable(
    ...
    frequency_range = np.arange(synthetizer_frequency_min, synthetizer_frequency_max + synthetizer_frequency_step, synthetizer_frequency_step) #frequency loop
    ...
    for f in frequency_range: #frequency loop
        ...
        values.append([str(f), unit.return_unit_str(frequency_output_unit), str(fixed_output_level), data_now])

        count +=1
        if not createprogressdialog is None:
            import wx
            wx.MicroSleep(500)
            message = "{lo_freq} {lo_pow}dB".format(lo_freq = str(f) + unit.return_unit_str(frequency_output_unit), lo_pow = str(fixed_output_level))
