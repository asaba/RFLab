...
def create_calibration_cable(
    ...
    for f in frequency_range:  # frequency loop
        ...
        values.append(
            [str(unit.convertion_from_base(f, frequency_output_unit)), unit.return_unit_str(frequency_output_unit),
        ...
        if not createprogressdialog is None:
            import wx
            wx.MicroSleep(500)
            message = "{lo_freq} {lo_pow}dB".format(lo_freq=current_frequency_human_readable,
