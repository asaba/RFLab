...
from measure_scripts.csvutility import frequency_LO_index, unit_LO_index, power_LO_index, is_LO_calibrated_index, \
    frequency_RF_index, unit_RF_index, power_RF_index, is_RF_calibrated_index, frequency_IF_index, unit_IF_index, \
    power_IF_index, is_IF_calibrated_index, n_LO_index, m_RF_index, conversion_loss, csv_file_header
import touchstone
...
def opentouchstonefile(data_file_name, filter_label=[]):
    result = []
    if filter_label:
        filter_label = [f_l.upper() for f_l in filter_label]
    newfile = touchstone.touchstone(data_file_name)
    parameters = newfile.get_sparameter_data(format="db")
    freq = parameters["frequency"]
    params_keys = parameters.keys()
    params_keys.remove("frequency")
    for k in params_keys:
        if filter_label is None:
            for index in range(len(freq)):
                result.append([k, freq[index], parameters[k][index]])
        elif k.upper() in filter_label:
            for index in range(len(freq)):
                result.append([k, freq[index], parameters[k][index]])

    return result, params_keys
...
def splitSpuriusCfiletablevalueDict(data_file_name, graph_type, table_value, sort_data, group_level_01, SD_LO_Frequency,
                                    SD_LO_Level, SD_RF_Level, SD_IF_Min_Level, savefile=True):
    """
...
def order_and_group_data(data_file_name,
    ...
    if graph_type in graph_types.values() + ["IP1"]:
        ...
        if graph_type == "GG":
            ...
            unit_value = unit.Hz
            data_file_directory = os.path.dirname(data_file_name)
            file_table_result, dummy = opentouchstonefile(data_file_name, IF_Frequency_selected)
    ...
    if graph_type == "LO":
        ...
        sort_data = [m_RF_index, frequency_RF_index, power_RF_index]
    ...
    return splitSpuriusCfiletablevalueDict(data_file_name, graph_type, file_table_result, sort_data, group_level_01,
                                           SD_LO_Frequency=SD_LO_Frequency, SD_LO_Level=SD_LO_Level,
                                           SD_RF_Level=SD_RF_Level, SD_IF_Min_Level=SD_IF_Min_Level,
                                           savefile=savefile), data_file_directory, graph_group_index, x_index, y_index, z_index, legend_index
...
class Graph_Axis_Range(Frequency_Range):
    ...
    def return_ticks_range(self, round_value=2, not_round=False):
        if not_round:
            ...
            return [round(x, round_value) for x in
                    np.arange(self.from_base().min, self.from_base().max + self.from_base().step,
                              self.from_base().step)]
