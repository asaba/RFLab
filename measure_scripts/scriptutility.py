...
def calibrate(input_power, input_frequency, input_frequency_unit, calibration_table, calibration_function=None,
              calibration_function_unit=None, round_freq=False):
...
class Frequency_Range(Generic_Range):
    ...
    def from_base(self):
        if self.based and self.unit != unit.dB and self.unit != unit.dBm:
            return Frequency_Range(unit.convertion_from_base(self.min, self.unit),
                                   unit.convertion_from_base(self.max, self.unit),
                                   unit.convertion_from_base(self.step, self.unit), self.unit)
        else:
            return self
