...
from guiobjects import return_file_browse, return_instrument, return_test_instrument, return_min_max_step_labeled, \
    return_usb_instrument, return_checkbox_labeled, return_comboBox_unit, return_textbox_labeled, return_spinctrl
from utilitygui import check_value_min_max, check_value_not_none, check_value_is_IP, create_instrument, \
    check_instrument_comunication, check_USB_instrument_comunication
...
class InstrumentPanelClass(wx.Panel):
    def __init__(self, parent):
        ...
        self.instrument_txt_IP, self.instrument_txt_Port, self.instrument_txt_Timeout, self.combobox_instrtype, self.instrument_sizer = return_instrument(
            self)
        self.instrument_test_button, self.instrument_label, self.instrument_test_sizer = return_test_instrument(self)
        self.instrument_enable_status, self.instrument_enable_status_sizer = return_checkbox_labeled(self,
                                                                                                     "Instrument Status")
        self.instrument_test_button.Bind(wx.EVT_BUTTON, self.OnTestInstrument)

    def OnTestInstrument(self, event):
        dummy, response = check_instrument_comunication(self.instrument_txt_IP.GetValue(),
                                                        self.instrument_txt_Port.GetValue(),
                                                        eval(self.instrument_txt_Timeout.GetValue()),
                                                        self.combobox_instrtype.GetValue())
...
class InstrumentUSBPanelClass(wx.Panel):
    def __init__(self, parent):
        wx.Panel.__init__(self, parent=parent, id=wx.ID_ANY)
        self.instrument_combobox_com_port, self.instrument_txt_timeout, self.instrument_combobox_baud, self.instrument_search_button, self.instrument_sizer = return_usb_instrument(
            self)
        self.instrument_test_button, self.instrument_label, self.instrument_test_sizer = return_test_instrument(self)
        self.instrument_enable_status, self.instrument_enable_status_sizer = return_checkbox_labeled(self,
                                                                                                     "Instrument Status")
    ...
    def OnTestInstrument(self, event):
        dummy, response = check_USB_instrument_comunication(self.instrument_combobox_com_port.GetValue(),
                                                            eval(self.instrument_txt_timeout.GetValue()),
                                                            self.instrument_combobox_baud.GetValue())
...
class TabPanelFSV(InstrumentPanelClass):
    ...
    def __init__(self, parent, enable_show_function=False):
        ...
        # gainAmplifier = 40 #dB
        self.gainAmplifier, dummy, dummy, self.sizer_gainAmplifier, dummy, dummy = return_textbox_labeled(self,
                                                                                                          "Gain Amplifier")
        ...
        self.threshold_power, dummy, dummy, self.sizer_threshold_power, dummy, dummy = return_textbox_labeled(self,
                                                                                                              "Threshold")
        ...
        self.spectrum_analyzer_central_frequency, self.spectrum_analyzer_central_frequency_unit, dummy, self.sizer_spectrum_analyzer_central_frequency, self.spectrum_analyzer_central_frequency_button, dummy = return_textbox_labeled(
            self, "Central Frequency", unit=True, enabled=False, enable_text="", read=True, button_text="Plot display")
        self.spectrum_analyzer_central_frequency_button.Bind(wx.EVT_BUTTON, self.OnPlotSpectrum)

        # FSV_delay = 1
        self.FSV_delay, dummy, dummy, self.sizer_FSV_delay, dummy, dummy = return_textbox_labeled(self,
                                                                                                  "Measure delay (s)")
    ...
    def OnPlotSpectrum(self, event):
        ...
        try:
            if self.Parent.GrandParent.runmodeitem.IsChecked():
                dlg = wx.MessageDialog(None, 'Test mode. Instruments comunication disabled', "Test mode",
                                       wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                self.testmode = True
        except:
        ...
        try:
            FSV = create_instrument(spectrum_analyzer_IP, spectrum_analyzer_Port, eval(spectrum_analyzer_Timeout),
                                    spectrum_analyzer_instrType, TEST_MODE=self.testmode, instrument_class="FSV",
        ...
        axis_y_legend = "Ref.Level {ref_level} dBm  - Attenuation {attenuation} dB".format(
            ref_level=str(spectrum_analyzer_IF_relative_level) if spectrum_analyzer_IF_relative_level_enable else "0",
...
class TabPanelPM5(InstrumentUSBPanelClass):
    ...
    def __init__(self, parent):
        ...
        self.pm5_misure_delay, dummy, dummy, self.sizer_pm5_misure_delay, dummy, dummy = return_textbox_labeled(self,
                                                                                                                "Measure delay (s)")
